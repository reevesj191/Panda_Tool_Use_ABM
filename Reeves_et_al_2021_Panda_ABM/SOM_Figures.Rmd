...
ggplot(tools_sum[tools_sum$treessdie == 1,], aes(x = min_d2_source, y= n_tools)) + 
  ...
  facet_grid(as.factor(`N Sources`)~as.factor(`N Trees`), labeller = label_both)+
  base_theme
...
ggplot(OGs, aes(x = d2source, y=Tool_size)) + geom_point()+
  facet_grid(as.factor(`N Sources`)~as.factor(`N Trees`))+
