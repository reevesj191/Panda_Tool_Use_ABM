...
acknowledgements: |
  We thanks the Max Planck Society for supporting this research. We also thank the Max Planck Institute for Evolutionary Anthropology for hosting our research group. JR thanks the Luke Premo for providing feedback and thoughtful discussion on an earlier version of this work.   
keywords:
  - Tool Transport
  - Niche Construction
  - Percussive tools
...
\caption{A two-way interaction plot illustrating the effect of the number of _Trees_ and number of _Sources_ on distance maximum distance a _Pounding Tool_ is able to move from where it was initially acquired, its source}
...
brks <- seq(from = 0, to= max(env_data$trees_available), by=100)

a <- ggplot(data = static, aes(ts, y=run_id)) + geom_tile(aes(fill = as.numeric(trees_available))) +base_theme + 
  scale_fill_viridis_c(breaks= brks)+
  theme(legend.pos = "none",                                                                               axis.text.y = element_blank())

b <- ggplot(data = dynamic, aes(ts, y=run_id)) + geom_tile(aes(fill = as.numeric(trees_available))) +
  scale_fill_viridis_c(breaks= brks)+
  base_theme + theme(legend.pos = "none",                                                                               axis.text.y = element_blank())
...
lay_mat <- rbind(c(1,1,1,3),
                 c(2,2,2,4))


grid.arrange(a,b,c,d, layout_matrix = lay_mat)
...
tool_sum_sub$tree_status <- NA

for(i in 1:nrow(tool_sum_sub)){
