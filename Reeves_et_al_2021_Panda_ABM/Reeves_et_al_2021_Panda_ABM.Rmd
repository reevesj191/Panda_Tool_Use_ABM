...
```{r figure 1, fig.height=6, fig.width=6.5, include=FALSE}


x <- OGs_sum[OGs_sum$treessdie == 1,]$num_nutree
y <- OGs_sum[OGs_sum$treessdie == 1,]$num_sources
z <- OGs_sum[OGs_sum$treessdie == 1,]$max_d2_source
...
points(trans3d(x=x, y=y, z=z, pmat=tmat))
...
```{r figure 2, fig.height=2, fig.width=3.5, message=FALSE, warning=FALSE, include=FALSE, fig.cap= "Time-series plots showing the changing in the number of Tool-use locations over time in model run where the number of Sources is 500 and the number of Trees is 2000. Each line in the plots to the right represents an individual iteration of model with time represented on the x-axis and the number of tool-use locations is represented on the y-axis. Note that in iterations where tree locations is dynamic the number of tool-use locations is always greater. The slope of the lines (right) show that the number of tool-using locations will continue to increase and would eventually plateau only after all 2000 Trees became available for tool-use. \\label{tree_death}"}
...
c <- ggplot(static, aes(x = ts, y = as.numeric(trees_available))) +
  ...
  labs(x = "Time", y = "N Tool-use Opportunities")+
  base_theme
...
c <- ggplot(OGS_sub, aes(x = d2source, y=Tool_size)) + 
  geom_point()+
...
d <- ggplot(run_tools_sum[run_tools_sum$treessdie == 1,], aes(x = as.factor(num_nutree),
                          y = per_site_pounding, 
