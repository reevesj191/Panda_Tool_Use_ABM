...
from abm_functions import  ClosestAgent, get_random_alphanumeric_string, add_tree_data, add_tool_data, connect_db
...
class PrimAgent(Agent):
    ...
    def CheckForStone(self):
        ...
        if self.model.condition == "Panda":

            threshold = 2000

        elif self.model.condition == "Reduced":

            threshold = 10

        elif self.model.condition == "FxJj50":

            threshold = 10
        
        elif self.model.condition == "FxJj20E":

            threshold = 34

        else:
            print("ERROR: No valid Condition")

        sources = [obj for obj in neighborhood if isinstance(obj, StoneSource)]  # Subsets out stones
        pounding = [obj for obj in neighborhood if isinstance(obj,PoundingTool) and obj.Tool_size >= threshold]  # Subsets out Pounding tools
    ...
    def SelectStone(self, stone_list, selection_method):
        ...
        if selection_method == 'random':
            ...
            nearest = ClosestAgent(self.pos,list=stone_list) # It determine the index position of the object that is
                                                             # located closest to the PrimAgent
    ...
    def UseStone(self, stone):
        ...
        if type(stone) is StoneSource:
            #print(isinstance(stone, PoundingTool))
            ...
            # PANDA FLAKER: Determining the size of the tool according to the conditions specified by the user. 
            
            if self.model.condition == "Panda":

                while self.Tool_size < 2000: # A while loop is used to ensure that generated tools have a size of greater than 2000 grams.
                ...
                while self.Tool_size < 10:
                    self.Tool_size = random.normal(125,92,1)[0] # Size of the tool is randomly drawn from a normal distribution
                                                               # with the mean and sd of tools found in the Tai forest
            elif self.model.condition == "FxJj50":

                while self.Tool_size < 10:
                    self.Tool_size = random.normal(125,92,1)[0]

            elif self.model.condition == "FxJj20E":

                while self.Tool_size < 34:
                    self.Tool_size = random.normal(218,197,1)[0]

            else:
                print("ERROR: No Correct Condition specified!")
        ...
        # Handling transport and use when a the object is a prexisting pounding tool.
            ...
            # PANDA FLAKER: determines whether a stone will break or not. probability set by the conditions of the model
            
            if self.model.condition in ["Panda", "Reduced"]:

                Break_prob = random.randint(1,100) # randomly draw an integer from 1 to 0
            
            elif self.model.condition in ["FxJj50", "FxJj20E"]:
            ...
            if Break_prob <= 25 + stone.rm_quality:
                ...
                if self.model.mem_safe is True:
                    
                    # JHE Revision
                    uses = rand.randint(1,self.model.max_uses_per_move)
                    a = 1

                    while a <= uses and stone.active is True:
                        self.Break_Stone_memory_safe(stone)
                        a += 1
                        if stone.active is False:
                                        ## Remove it from model
                            self.model.schedule.remove(stone)
                            self.model.grid.remove_agent(stone)
    ...
    def BreakStone(self, stone):
        ...
        if self.model.condition == "Panda":
            ...
            while fragment >= stone.Tool_size / 2:
                fragment = random.exponential(10, 1)[0]
            ...
            while fragment >= stone.Tool_size or fragment < 1: # This ensures that the flake is not larger than the core
                fragment = random.normal(5.6,8.7,1)[0]
        
        elif self.model.condition == "FxJj20E":

            while fragment >= stone.Tool_size or fragment < 1: # This ensures that the flake is not larger than the core
                fragment = random.normal(10,15.9,1)[0]
        else:
        ...
        ## PANDA FLAKER: if the tool size remains larger than 2000 then it remains active as a useable tool.
        ...
        if stone.Tool_size < threshold: ## if it is less than 2000 it is considered exhausted and becomes no longer useable
    ...
    def Break_Stone_memory_safe(self, stone):
        ...
        if self.model.condition == "Panda":
            ...
            while fragment >= stone.Tool_size or fragment < 0: # This ensures that the flake is not larger than the core
                fragment = random.normal(5.6,8.7,1)[0]
        ...
        if fragment >= threshold:
            ...
            new_tool.sequence = stone.n_uses
            # Adds the new fragment to the model space.
            self.model.grid.place_agent(new_tool, self.pos)
            self.model.schedule.add(new_tool)
            ...
            active = False
            tool_id = get_random_alphanumeric_string(8)
            conn = connect_db(self.model.sql)  # Open connection with SQL database
            ...
            row = [self.model.run_id,
                   ...
                   parent_id,
                   stone.source_id,
                   self.pos[0],  # Must inherit its location from the primAgent
                   self.pos[1],  # Must inherit its location from the primAgent
                   fragment,
                   fragment,
                   active,
                   stone.rm_quality,
    ...
    def step(self):
        ...
        if self.NutTreeLoc != -1:
            ...
            if len(stones) > 0:
                ...
                stone = self.SelectStone(stones,
                                         selection_method=self.model.tool_acquistion)
...
class NutTree(Agent):
    ...
    def amIavailable(self):
        ...
        neighborhood = self.model.grid.get_neighborhood(self.pos,
                                                        radius= 3,
                                                        moore=True,
                                                        include_center=True)  # Defines the
        # neighborhood around the agent
        neighborhood = self.model.grid.get_cell_list_contents(
            neighborhood)  # Returns a list of the objects in the neighborhood
        ...
        pounding_tools = [obj for obj in pounding_tools if obj.Tool_size >= threshold]
...
class PoundingTool(Agent):
    def __init__(self, unique_id, model, tool_size, active, s_id, parent_id="OG", q=-1, born=-1):
        ...
        self.ts_died = -1
        self.n_uses = 0
        self.sequence = -1   # For fragments only, it is the order number that the fragment came off the pounding tool.  
